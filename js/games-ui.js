...
const GamesUI = {
  ...
  openGame(id) {
    ...
    if (!m) return;
    m.innerHTML = '<div class="modal-content game-modal-content">' +
      '<div class="modal-header" style="background: ' + g.color + '">' +
      '<button class="modal-close" onclick="GamesUI.closeModal()">X</button>' +
      '<div class="modal-game-icon">' + g.icon + '</div>' +
      '<h2>' + g.name + '</h2><p>' + g.desc + '</p></div>' +
      '<div class="modal-body"><h3 class="section-title">Pilih Nominal</h3>' +
  ...
  showOrderForm() {
    ...
    m.innerHTML = '<div class="modal-content game-modal-content">' +
      ...
      '<option value="shopeepay">ShopeePay</option></select></div>' +
      '<button class="btn-primary btn-full" onclick="GamesUI.submitOrder()">Pesan</button>' +
  ...
  submitOrder() {
    ...
    const order = {
      ...
      userId: uid, payment: pay, date: new Date().toISOString(),
    };
  ...
  showSuccess(o) {
    const m = document.getElementById('game-modal');
    if (!m) return;
    m.innerHTML = '<div class="modal-content game-modal-content"><div class="modal-body success-body">' +
      '<div class="success-icon">OK</div><h2>Pesanan Berhasil!</h2>' +
      ...
      '</div>' +
      '<div class="reward-badge"><span>+10 XP</span><span>+5 Gems</span></div>' +
