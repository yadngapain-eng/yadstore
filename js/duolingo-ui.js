...
const DuoUI = {
  ...
  renderStats() {
    ...
    if (pe) pe.style.width = ((level.currentXp / level.neededXp) * 100) + '%';
    const li = el('level-info');
    if (li) li.textContent = `${level.currentXp} / ${level.neededXp} XP`;
  },
  ...
  renderCategoryTabs() {
    ...
    c.innerHTML = Object.keys(cats).map(k => {
      ...
      return `<button class="cat-tab ${act}" onclick="DuoUI.switchCategory('${k}')" style="--cat-color: ${cat.color}">
        <span class="cat-icon">${cat.icon}</span><span class="cat-label">${cat.label}</span>
      </button>`;
    }).join('');
  },
  ...
  startLesson(id) {
    ...
    if (hearts <= 0) { alert('❤️ Hearts habis! Tunggu 4 jam atau isi ulang (50 💎).'); return; }
  ...
  exitLesson() {
    ...
    if (typeof DuoUI.renderProfile === 'function') DuoUI.renderProfile();
  },
  ...
  renderAchievements() {
    ...
    c.innerHTML = a.map(x => `
      <div class="achievement-card ${x.unlocked ? 'unlocked' : 'locked'}">
        <div class="achievement-icon">${x.unlocked ? x.icon : '🔒'}</div>
        <div class="achievement-info"><h4>${x.title}</h4><p>${x.desc}</p></div>
      </div>`).join('');
  },
